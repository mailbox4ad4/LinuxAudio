...
void generate_sine(const snd_pcm_channel_area_t *areas, snd_pcm_uframes_t offset, int count, double &phase)
{
    const double max_phase = 2. * M_PI;

    const double step = max_phase * freq / rate; // phase step
    ...
    const bool big_endian = snd_pcm_format_big_endian(format) == 1;
    const bool to_unsigned = snd_pcm_format_unsigned(format) == 1;
    const bool is_float = (format == SND_PCM_FORMAT_FLOAT_LE || format == SND_PCM_FORMAT_FLOAT_BE);
...
int async_loop(snd_pcm_t *handle, signed short *samples, snd_pcm_channel_area_t *areas)
{
...
int async_direct_loop(snd_pcm_t *handle, signed short *samples ATTRIBUTE_UNUSED, snd_pcm_channel_area_t *areas ATTRIBUTE_UNUSED)
{
    ...
    data.samples = nullptr; // we do not require the global sample area for direct write
    data.areas   = nullptr; // we do not require the global areas for direct write
    data.phase   = 0;
...
int direct_loop(snd_pcm_t *handle, signed short *samples ATTRIBUTE_UNUSED, snd_pcm_channel_area_t *areas ATTRIBUTE_UNUSED)
{
    double phase = 0;
    const snd_pcm_channel_area_t *my_areas;
    snd_pcm_uframes_t offset, frames, size;
...
struct transfer_method transfer_methods[] =
{
    ...
    { nullptr,                 SND_PCM_ACCESS_RW_INTERLEAVED     , nullptr             }
};
...
void help()
{
    ...
    for (int k = 0; transfer_methods[k].name; ++k)
    {
...
int main(int argc, char *argv[])
{
    struct option long_option[] =
    {
        ...
        {"device"     , 1, nullptr, 'D'},
        {"rate"       , 1, nullptr, 'r'},
        {"channels"   , 1, nullptr, 'c'},
        {"frequency"  , 1, nullptr, 'f'},
        {"buffer"     , 1, nullptr, 'b'},
        {"period"     , 1, nullptr, 'p'},
        {"method"     , 1, nullptr, 'm'},
        {"format"     , 1, nullptr, 'o'},
        {"verbose"    , 1, nullptr, 'v'},
        {"noresample" , 1, nullptr, 'n'},
        {"pevent"     , 1, nullptr, 'e'},
        {nullptr      , 0, nullptr,  0 }
    };
    ...
    while (1)
    {
        ...
        if ((c = getopt_long(argc, argv, "hD:r:c:f:b:p:m:o:vne", long_option, nullptr)) < 0)
        {
            break;
        }
        ...
        switch (c)
        {
            ...
            case 'm':
            {
                ...
                if (transfer_methods[method].name == nullptr)
                {
                    method = 0;
                }
                break;
            ...
            case 'o':
            {
                ...
                {
                    format = SND_PCM_FORMAT_S16; // default format
                }
